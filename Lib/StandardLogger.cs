        ...
        public static StandardLogger Create(Ambiente ambiente)
        {
            switch (ambiente)
            {
                case Ambiente.NetWebApi:
                    return new StandardLogger(SerilogFactory.CreateLegacyWebApiLogger("Server=127.0.0.1,1401;Database=ExampleDb;User Id=SA;Password=asdf1234$;TrustServerCertificate=True;"));

                default:
                    return new StandardLogger("Server=127.0.0.1,1401;Database=ExampleDb;User Id=SA;Password=asdf1234$;TrustServerCertificate=True;");
            }
        }
    }
}